      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {products.map(p => (
          <Link key={p.id} href={`/site/${tenant}/store/p/${p.handle}`} className="group cursor-pointer">
            <div className="aspect-[4/5] bg-white/10 rounded-2xl mb-4 overflow-hidden flex items-center justify-center relative group-hover:shadow-md transition-all">
              {p.imageUrl ? (
            ...
            <h3 className="font-semibold text-lg text-white group-hover:text-white">{p.title}</h3>
            <p className="text-slate-400 font-medium">${Number(p.variants[0]?.price || 0).toFixed(2)}</p>
          </Link>
        ))}
        {products.length === 0 && (
          <p className="text-slate-400 col-span-4">Esta tienda aún no tiene productos disponibles.</p>
        )}
      </div>
    </div>
  );
