  ...
  if (successOrderId) {
    return (
      <div className="max-w-xl mx-auto mt-24 text-center">
        ...
        <Link href={`/site/${tenant}/store`} className="px-6 py-3 bg-gradient-to-r from-cyan-500 to-purple-600 shadow-lg shadow-cyan-500/20 text-white font-medium rounded-lg hover:from-cyan-400 hover:to-purple-500 hover:shadow-cyan-400/40">
          Volver a la Tienda
        </Link>
      </div>
    );
    ...
    <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-12">
      ...
      <div className="md:w-1/2">
        ...
        <div className="bg-white/5 rounded-2xl p-6">
          ...
          <div className="pt-6 mt-6 border-t border-white/10 flex justify-between items-center text-xl font-bold">
            <span>Total</span>
      ...
      <div className="md:w-1/2">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 glass-card border-white/5 border border-white/10 rounded-2xl p-8">
          <div>
            <label className="block text-sm font-medium text-slate-300">Nombre Completo</label>
            <input required type="text" value={customerName} onChange={e => setCustomerName(e.target.value)} className="mt-1 block w-full rounded-md border-white/10 shadow-sm p-3 border focus:border-cyan-400 focus:ring-cyan-400 focus:ring-black" />
          </div>
          
          <div>
            <label className="block text-sm font-medium text-slate-300">Correo Electrónico</label>
            <input required type="email" value={customerEmail} onChange={e => setCustomerEmail(e.target.value)} className="mt-1 block w-full rounded-md border-white/10 shadow-sm p-3 border focus:border-cyan-400 focus:ring-cyan-400 focus:ring-black" />
          </div>

          <div className="pt-4 border-t border-gray-100">
            <h3 className="font-medium text-white mb-4">Método de Pago</h3>
          ...
          <button 
            ...
            className="w-full bg-gradient-to-r from-cyan-500 to-purple-600 shadow-lg shadow-cyan-500/20 text-white rounded-xl py-4 font-bold text-lg hover:from-cyan-400 hover:to-purple-500 hover:shadow-cyan-400/40 transition-colors disabled:opacity-50"
          >
