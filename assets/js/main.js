// main.js - Expertos MKD Web Interactivity
...
function initHeroSlider() {
  ...
  function showSlide(index) {
    ...
    // Update slides visibility and animation states
...
function initLeadForm() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    // Simulate API Call (cPanel / Webhook ready)
    setTimeout(() => {
      // Revert loading state
      submitBtn.disabled = false;
      submitBtn.innerHTML = originalBtnText;

      // Show customized feedback modal
  ...
  const modalHtml = `
    <div id="success-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm transition-opacity duration-300">
      <div class="glass-panel max-w-md w-full rounded-2xl p-8 border border-white/10 shadow-2xl transform transition-transform duration-300 scale-95 opacity-0" id="success-modal-content">
        ...
        <h3 class="text-2xl font-heading font-bold text-center text-white mb-2">¡Todo listo, ${clientName}!</h3>
        <p class="text-slate-300 text-center text-sm mb-6 leading-relaxed">
          Hemos recibido tus datos de forma segura. Tu reto de crecimiento ya está bajo análisis de nuestros expertos.
        </p>
        <div class="space-y-3">
          ...
          <a href="#" id="modal-calendly-btn" class="w-full py-3 bg-gradient-to-r from-cyan-500 to-purple-600 hover:from-cyan-400 hover:to-purple-500 text-white font-medium rounded-lg transition text-center block text-sm shadow-md shadow-cyan-500/10">
            Agendar Llamada Ahora (Calendly)
          </a>
        </div>
      </div>
    </div>
...
function openCalendlyScheduler() {
      ...
      <div class="glass-panel w-full max-w-4xl h-[85vh] rounded-2xl border border-white/10 shadow-2xl flex flex-col overflow-hidden transform transition-transform duration-300 scale-95 opacity-0" id="calendly-modal-content">
        ...
        <div class="px-6 py-4 border-b border-white/5 flex justify-between items-center bg-slate-950/60">
          <div class="flex items-center gap-2">
            ...
            <h3 class="text-lg font-heading font-bold text-white">Agendar Auditoría de Growth & Tech</h3>
          </div>
        ...
        <div class="flex-grow bg-slate-900 relative">
          <!-- Loading screen inside iframe container -->
          <div id="iframe-loader" class="absolute inset-0 flex flex-col items-center justify-center bg-slate-900 z-10">
            ...
            <span class="text-xs text-slate-400 tracking-widest uppercase">Cargando Agenda de Expertos...</span>
          </div>
          <!-- Real Calendly integration, fallback to a clean iframe mockup if needed, but here we place a real calendar placeholder -->
          <iframe 
            src="https://calendly.com/expertosmkd-demo/15min?embed_domain=expertosmkd.com&embed_type=Inline" 
            width="100%" 
            height="100%" 
            frameborder="0"
            id="calendly-iframe"
  ...
  // Backup timeout: If Calendly takes too long (or blocked/offline), show a simulated success screen or just clear loader
  setTimeout(() => {
    if (document.getElementById('iframe-loader')) {
      loader.innerHTML = `
        <p class="text-sm text-slate-300 text-center px-6 mb-4">¿Tuviste problemas cargando la agenda externa?</p>
...
function initWhatsAppBadge() {
  ...
  // Toggle tooltip manually on click if needed
